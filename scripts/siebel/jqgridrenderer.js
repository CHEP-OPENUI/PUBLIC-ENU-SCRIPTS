...
if ( typeof ( SiebelAppFacade.JQGridRenderer ) === "undefined" ) {
    ...
    define("siebel/jqgridrenderer", ["order!3rdParty/jqGrid/current/js/i18n/grid.locale-en", "order!3rdParty/jqGrid/current/js/jquery.jqGrid.min", "order!3rdParty/jqgrid-ext", "order!siebel/phyrenderer"],
      function () {
          ...
          function OnCloseDatePicker(dateElement,ctrl,inputText,isEscapeKey){
              ...
              if (ctrlType === consts.get("SWE_CTRL_DATE_TZ_PICK") ||
                ctrlType === consts.get("SWE_CTRL_DATE_TIME_PICK")) {
                  /* 
                   * In the time picker addon plugin, on close we are sending the ISO format whereas for
                   * Date picker we are sending the local format. Till the addon plugin fixes this issue
                   * we need the below fix.
                   */
                  dateElement.val(inputText);
          ...
          function MoveToPreviousRow( e ) {
            ...
            if( e.shiftKey && key === 9 ){
              ...
              if( iRow >= 0 ){
                ...
                setTimeout(function () {
                  that.GetGrid().jqGrid('editCell', that.GetSelectedRow(), iCol, true);
                }, 10);
              }
            }
          }
          ...
          function MoveToNextRow( e ){
            ...
            if( !e.shiftKey && key === 9 ){
              ...
              if( iRow <= this.GetRowCount() ){
                ...
                setTimeout(function () {
                    that.GetGrid().jqGrid('editCell', that.GetSelectedRow(), iCol, true);//fix for current row being changed in settimeout due to delay.
                }, 10);
              }
              else{
                setTimeout( function(){
          ...
          function PrepareColumnInfo( listOfColumns ) {
            ...
            for( var index = 0, length = listOfColumns.length; index < length; index++ )
            {
              ...
              switch( listOfColumns[index].controlType ){
                            ...
                            click     : function( ctrl, tgt ){
                                ...
                                SiebelAppFacade.ControlBuilder.DatePick({
                                    ...
                                    onClose   : function ( inputText, inst) {
                                        OnCloseDatePicker.call(self,$(this),ctrl,inputText,inst.isEscapeKey);
                                    }
                                });
                            }
                        });
               ...
               case calc:
                ...
                case textArea:
                  ...
                  current.editoptions = {"style" :textAreaHeight , "class" : "siebui-list-textareactrl", "maxlength": (listOfColumns[index].control.GetMaxSize() || 256)};
                  break;
              ...
              if( listOfColumns[index].control.GetShowBase() &&
                ...
                listOfColumns[index].control.GetUIType() != mailTo) {
              ...
              columnName.push(HtmlEncode(listOfColumns[index].control.GetDisplayName()));//Bug#18598494
            }
          ...
          function HandleTabKey (el, control, event) {
              ...
              if (control.GetUIType() === combo) {
                  var iRow = el.attr("id").substr(0, el.attr("id").indexOf("_"));
                  var iCol = el.parent("td").parent("tr").children().index(el.parent("td"));
                  el.data('inLeaveField', true);
                  retVal = !(InvokeColumnChange.call(this, el, control, iRow, iCol, $(el).val()));
                  el.removeData('inLeaveField');
              }
              return retVal;
          }
     ...
     *  */
            ...
            if(jqGridCtrl.length>0)
            {
               ...
               try {
                  var gridConfig = {
                    ...
                    if( utils.IsTrue(SiebelApp.S_App.IsAutoOn() ) ){//QTP attributes
                   ...
                   tableHeader = null;
                    ...
                    utils.Alert (error); // We need a better logging mechanism...
                }
            }
             jqGridCtrl = null;
             columnInfo = null;
          ...
          function InvokeColumnChange (el, ctrl, iRow, iCol, value){
            ...
            return this.GetPM().OnControlEvent(siebConsts.get("PHYEVENT_COLUMN_BLUR"), iRow, ctrl, value);
          }
          ...
          function BindColInputEvents( el, iRow, iCol ,control){
              ...
              .bind( "mousedown", { ctx: this }, function(event){
                  if( $( this ).attr( "type" ) === "checkbox" ){
                      ...
                      $( this ).data( "readonly-value", $( this ).prop( "checked" ) );
                  }
              })
              .bind("blur", { ctx: this, row: iRow, col: iCol, control: control }, function (evt) {
                  if ( el.data('inLeaveField') ) {
                      return;
                  }
                  var ctrl = $( this  );
              ...
              .bind( "click", { ctx: this, row: iRow, col : iCol }, function( evt ){
                  ...
                  if ( typeof attr !== 'undefined' && attr !== false ) {
                      el.prop( "checked", el.data( "readonly-value") !== undefined ? el.data( "readonly-value") : !el.prop( "checked" ) );
                      el.removeData( "readonly-value");
                  }
         ...
         function HandleDrillDown( evt ){
              ...
              if( controlType === combo ){
                  ctrlEl
                      ...
                      .bind( "autocompleteclose", { ctx : self, ctrl : control }, function ( evt){
                          ...
                          var pendingValue = $(this).data("pendingSelect");
                          if (pendingValue) {                              
                              $(this).removeData("pendingSelect");
                              var retVal = evt.data.ctx.GetPM().ExecuteMethod("LeaveField", evt.data.ctrl, pendingValue, true);                              
                              if (retVal === false) {
                                  return retVal;
                              }
                          }
                      ...
                      .bind( "autocompleteselect", { ctx : self, ctrl : control }, function ( evt, ui){ // Bug#16385076
                          ...
                          $(this).data("pendingSelect", ui.item.value);                          
                      });
              }
          ...
          function GridAfterSaveCell(rowid, cellname, value, iRow, iCol) {
              var self = $(this).data("jqGrid"),
                  el = self.GetGrid().find("#" + iRow + "" + cellname);
          ...
          function GridBeforeSaveCell( rowid, cellname, value, iRow, iCol ) {
              ...
              var currentElem = $("#" + iRow + "_" + cellname, gridCtrl);
              if( ctrl ){
                  ...
                  currentElem.data('inLeaveField', true);
                  InvokeColumnChange.call(self, this, ctrl, iRow, iCol, value);
                  currentElem.removeData('inLeaveField');
          ...
          function GridAfterRestoreCell( rowid, value, iRow, iCol ) {
              ...
              if( !bc || (bc && bc.GetUpdateArray()) ||
                      self.GetPM().Get("GetSelection") === ( Number(iRow)- 1 + ( self.GetPM().Get("GetBeginRow")||0 ) ) ){
                  InvokeColumnChange.call (self, this, ctrl, iRow, iCol, value);
              }
          ...
          JQGridRenderer.prototype.SetControlValue = function (control, value, index) {
              this.SetCellValue(index ? index : this.GetSelectedRow(), control.GetName(), value);
          ...
          JQGridRenderer.prototype.SetCellValue = function(rowId,fieldName,newValue){
            ...
            var jqGridCtrl = this.GetGrid();
            fieldName = this.GetColumnHelper( ).GetModifiedColumnName( fieldName );
            var rowid = this.GetPM().Get("GetSelection") + 1 - (this.GetPM().Get("GetBeginRow")||0);
            var el = $( "#" + rowid + "_" + fieldName, jqGridCtrl );
                  ...
                  for (var key in map_col) {
                    if (map_col.hasOwnProperty(key)) {
                        if (listofColumns[i].name === map_col[key]) {
                            ...
                            if (canUpdate) {
                                setTimeout(function () {
                                    currentRow = that.GetSelectedRow();//fix for current row being changed in settimeout due to delay.
              ...
              for(var i=0;i < column_num;i++)
              {
                       if(listofColumns[i].name === controlName)
                       {
                          ...
                          if(canUpdate)
                          {
                            ...
                            setTimeout(function () {
                                currentRow = that.GetSelectedRow();//current row being changed in settimeout due to delay.
